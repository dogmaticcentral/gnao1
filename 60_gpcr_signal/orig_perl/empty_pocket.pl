...
($recovery) && print OUTF "unset key\n";
($recovery) || print OUTF "set xlabel 'seconds'\n";
($recovery) || print OUTF "set ylabel 'molecular population size (%)'\n";
...
if ($rootnm  ne "base") {
    ...
    print OUTF "set style line 3 lc rgb '#004c91'  lt 2 lw 2 dt '.' # --- blue\n";
    print OUTF "set style line 4 lc rgb '#ffc220'  lt 2 lw 2 dt '.' # --- yellow\n";
...
sub set_tweakable() {
	...
	$kf = "2.0";
	$kr = "0.1";
	$tweakable .= "f3_Gtrimer_to_GPCR_active:	\@c0:agonist(p_site!1).GPCR(Galpha,agonist!1) + \@c0:Galpha(GPCR,GnP~none,p_site!1,mut~mutant).Gbg(p_site!1) <-> ";
	$tweakable .= "\@c0:agonist(p_site!2).GPCR(Galpha!3,agonist!2).Galpha(GPCR!3,GnP~none,p_site!1,mut~mutant).Gbg(p_site!1)	 $kf, $kr	 \n";
}
...
10 \@c0:Galpha(GPCR,GnP~GDP,p_site,mut~mutant) 0.0
11 \@c0:Galpha(GPCR,GnP~GTP,p_site,mut~mutant) 0.0
12 \@c0:Galpha(GPCR,GnP~none,p_site,mut~mutant) 25.0
end seed species
...
";
	if ($recovery) {
	    $literal2 .= "setConcentration(\"\@c0:agonist(p_site)\", 6.0)";
	} else {
	    $literal2 .= "setConcentration(\"\@c0:agonist(p_site)\", 60.0)";
	}
	$literal2 .= "
simulate_ode({continue=>1,t_end=>20.1,n_steps=>3,atol=>1e-8,rtol=>1e-8,sparse=>1})
setConcentration(\"\@c0:AChE(agonist)\", 120.0)
simulate_ode({continue=>1,t_end=>220,n_steps=>200,atol=>1e-8,rtol=>1e-8,sparse=>1})
";
