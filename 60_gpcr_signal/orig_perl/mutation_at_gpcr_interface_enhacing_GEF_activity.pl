...
print OUTF "#set output 'nice_web_plot.svg'\n";
...
print OUTF "set xrange [15:75]\n";
print OUTF "set yrange [60:90]\n";
...
if ($rootnm  ne "base") {
    ...
    print OUTF "set style line 3 lc rgb '#004c91'  lt 2 lw 3 dt '.' # --- blue\n";
    print OUTF "set style line 4 lc rgb '#ffc220'  lt 2 lw 3 dt '.' # --- yellow\n";
...
sub set_tweakable() {
	...
	#note that the signal can be recovered by  setConcentration(\"\@c0:agonist(p_site)\", 60.0 --> set to 120
	$kf = "10.0";
	$kr = "0.1";
...
simulate_ode({t_end=>20,n_steps=>200,atol=>1e-8,rtol=>1e-8,sparse=>1})
";
	if ($recovery) {
	    $literal2 .= "setConcentration(\"\@c0:agonist(p_site)\", 50.0)";
	} else {
...
simulate_ode({continue=>1,t_end=>220,n_steps=>1000,atol=>1e-8,rtol=>1e-8,sparse=>1})
";
